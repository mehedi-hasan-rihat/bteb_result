...
const SEM_ORD: Record<number, string> = {
  1: "1ST",
  2: "2ND",
  3: "3RD",
  4: "4TH",
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-[#f0f0f0] flex flex-col overflow-x-hidden">
      ...
      <div className="border-b border-white/10 py-2 overflow-hidden bg-[#0a0a0a]">
        <div className="marquee-track">
          {[...MARQUEE_ITEMS, ...MARQUEE_ITEMS].map((item, i) => (
            <span
              key={i}
              className="text-[10px] font-bold uppercase tracking-[0.25em] text-white/30 px-6 shrink-0"
            >
      ...
      <section className="px-5 pt-10 pb-8 border-b border-white/10">
        <div className="max-w-lg mx-auto">
          ...
          <h1
            className="font-black uppercase leading-[0.88] mb-6 fade-up fade-up-2"
            style={{
              fontSize: "clamp(3.2rem, 14vw, 6rem)",
              letterSpacing: "-0.02em",
            }}
          >
            RESULT
            <br />
            <span style={{ color: "#e8ff00", WebkitTextStroke: "0px" }}>
              FINDER
            </span>
          </h1>
          ...
          >
            <div className="flex items-center">
              <span className="pl-4 text-white/30 font-mono text-sm shrink-0 select-none">
                #
              </span>
              <input
                ref={inputRef}
          ...
          <div className="flex flex-col gap-px fade-up fade-up-1">
            ...
            <div className="px-5 py-6" style={{ background: statusColor }}>
              <div className="flex items-start justify-between mb-4">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-black/50 mb-1">
                    Roll No.
                  </p>
                  <p
              ...
              <div className="flex items-center gap-3">
                <span
                  className="text-xs font-black uppercase tracking-[0.2em] px-3 py-1.5"
            ...
            {/* ── INSTITUTE ── */}
            <div
              className="bg-white/5 px-5 py-4 border-l-2"
              style={{ borderColor: "#e8ff00" }}
            >
              <p className="text-[9px] font-bold uppercase tracking-[0.3em] text-white/30 mb-1">
                Institute
              </p>
                  ...
                  {Object.entries(result.gpas)
                    ...
                    .map(([key, val]) => {
                      ...
                      return (
                        <div
                          key={key}
                          className="bg-[#0a0a0a] px-2 py-3 text-center"
                        >
                          <p className="text-[9px] font-bold uppercase tracking-widest text-white/30 mb-1">
                            {SEM_ORD[n]}
                          </p>
                          <p
                            className={`font-black font-mono text-base ${isRef ? "text-[#ff9500]" : "text-[#e8ff00]"}`}
                          >
                ...
                <div className="flex flex-col gap-px">
                  {result.ref_subjects.map((sub, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className="flex items-center justify-between bg-[#0a0a0a] px-3 py-3 gap-3"
                      >
      ...
      <div className="border-t border-white/10 py-2 overflow-hidden">
        ...
        >
          {[...MARQUEE_ITEMS, ...MARQUEE_ITEMS].map((item, i) => (
            <span
              key={i}
              className="text-[10px] font-bold uppercase tracking-[0.25em] text-white/15 px-6 shrink-0"
            >
      ...
      <footer className="border-t border-white/10 px-5 py-3 flex items-center justify-between">
        ...
        <Link
          href="/admin"
          className="text-[9px] font-bold uppercase tracking-[0.3em] text-white/20 hover:text-white/50 transition-colors"
        >
          Admin ↗
        </Link>
      </footer>
